...
Pour compiler ``demo-01.c`` dans un terminal sous Unix,
...
*gestion d'évènements* ; c'est pourquoi on donne la fonction ``func`` 
en 4e argument de :func:`ez_window_create`.

.. index:: Callback; Exemple

La fonction d'évènement ``func``, encore appelée *callback*, est une fonction 
...
à la souris. Les coordonnées des sommets sont mémorisées dans des variables
globales. Chaque fois que le bouton de la souris est cliqué, un sommet est

...
Installez d'abord ``XCode``, l'environnement de développement de Mac OS X,
depuis l'Apple Store ou encore depuis le DVD d'installation. 
Cette manipulation installe en même temps ``gcc``, ``make``, et les librairies 
``X11``.
...
Remarque :
    ...
    Sur d'anciennes versions de MacOS (10.4) il peut même être nécessaire
...
Modifiez ensuite le fichier Makefile_ en tapant :

.. code-block:: console

   nedit Makefile

Si ``nedit`` vous affiche ``NEdit: can't open display``, alors vous vous êtes
trompé de terminal, c'est dans le terminal XWin qu'il faut taper les commandes.

Dans ``nedit``, section A,  remplacez ``-std=c99`` par ``-std=gnu99`` ; enregistrez
puis quittez.

On peut enfin passez à la compilation de EZ-Draw : tapez

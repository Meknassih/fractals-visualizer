...
void draw_help (Ez_window win, Gui *gui)
{
    ...
    ez_draw_text (win, EZ_TC, gui->win1_w/2, y1+20,
        "Click on a block (of at least to bubbles\n"
...
void win1_onExpose (Ez_event *ev, Gui *gui)
{
    Game *game = gui->game;
    int sta = game->state;
...
void win1_onConfigureNotify (Ez_event *ev, Gui *gui)
{
    gui->win1_w = ev->width;
...
void win1_onButtonPress (Ez_event *ev, Gui *gui)
{
    Game *game = gui->game;
    int id_block;
...
void win1_onButtonRelease (Ez_event *ev, Gui *gui)
{
    Game *game = gui->game;
...
void win1_onKeyPress (Ez_event *ev, Gui *gui)
{
    Game *game = gui->game;
...
void win1_onTimerNotify (Ez_event *ev, Gui *gui)
{
    Game *game = gui->game;
...
void win1_event (Ez_event *ev)              /* Called for each event on win1 */
{
    ...
    switch (ev->type) {
        case Expose          : win1_onExpose          (ev, gui); break;
        case ConfigureNotify : win1_onConfigureNotify (ev, gui); break;
        case ButtonPress     : win1_onButtonPress     (ev, gui); break;
        case ButtonRelease   : win1_onButtonRelease   (ev, gui); break;
        case KeyPress        : win1_onKeyPress        (ev, gui); break;
        case TimerNotify     : win1_onTimerNotify     (ev, gui); break;
    }
}

...
void win1_onExpose (Ez_event *ev, App *app)
{
...
void win1_onKeyPress (Ez_event *ev, App *app)
{
    switch (ev->key_sym) {
        case XK_q : 
            ez_quit (); 
...
void win1_onKeyRelease (Ez_event *ev, App *app)
{
    switch (ev->key_sym) {
        case XK_Left :
        case XK_Right :
...
void win1_onTimerNotify (Ez_event *ev, App *app)
{
    if (app->state == S_CREATE) {
...
void win1_event (Ez_event *ev)              /* Called for each event on win1 */
{
    ...
    switch (ev->type) {
        case Expose        : win1_onExpose        (ev, app); break;
        case KeyPress      : win1_onKeyPress      (ev, app); break;
        case KeyRelease    : win1_onKeyRelease    (ev, app); break;
        case TimerNotify   : win1_onTimerNotify   (ev, app); break;
    }
}
...
void win2_onExpose (Ez_event *ev, App *app)
{
    int w, h;
...
void win2_onKeyPress (Ez_event *ev, App *app)
{
    Ez_event ev2 = *ev;
    ev2.win = app->win1;
    win1_onKeyPress (&ev2, app);
}


void win2_onKeyRelease (Ez_event *ev, App *app)
{
    Ez_event ev2 = *ev;
    ev2.win = app->win1;
    win1_onKeyRelease (&ev2, app);
}


void win2_onTimerNotify (Ez_event *ev, App *app)
{
    (void) ev;  /* unused */
...
void win2_event (Ez_event *ev)              /* Called for each event on win2 */
{
    App *app = ez_get_data (ev->win);

    switch (ev->type) {
        case Expose        : win2_onExpose        (ev, app); break;
        case KeyPress      : win2_onKeyPress      (ev, app); break;
        case KeyRelease    : win2_onKeyRelease    (ev, app); break;
        case TimerNotify   : win2_onTimerNotify   (ev, app); break;
    }
}
...
{
    app->win1 = ez_window_create (WIN1_W, WIN1_H, "Game of labyrinth", win1_event);
    app->win2 = ez_window_create (WIN2_W, WIN2_H, "3D View", win2_event);

...
void win1_onMotionNotify (Ez_event *ev)
{
    (void) ev;  /* unused */
}


void win1_onButtonPress (Ez_event *ev)
{
    Gui *gui = ez_get_data (ev->win);
    App *app = gui->app;
...
void win1_onButtonRelease (Ez_event *ev)
{
    Gui *gui = ez_get_data (ev->win);
    App *app = gui->app;
...
void win1_event (Ez_event *ev)              /* Called for each event on win1 */
{
    switch (ev->type) {
        ...
        case MotionNotify  : win1_onMotionNotify (ev); break;
        case ButtonPress   : win1_onButtonPress (ev); break;
        case ButtonRelease : win1_onButtonRelease (ev); break;
...
void gui_init (Gui *gui, App *app)
{
    ...
    gui->win1 = ez_window_create (WIN1_W, WIN1_H, "Sliding puzzle", win1_event);

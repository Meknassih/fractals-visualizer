...
void win1_on_expose(Ez_event *ev) {
  ...
  if (win1_data->zooming)
    dessiner_liste(ev->win, 1, win1_data->list_zoom, ez_red);
  else
...
void win1_on_keypress (Ez_event *ev) {
  ...
  case XK_l:
    ...
    break;
  case XK_i:
    win1_data->list = koch(WIDTH_MAIN, HEIGHT_MAIN, win1_data->n , win1_data->c);
    win1_data->configure_count = 0;
...
void win1_on_buttonpress(Ez_event *ev) {
  ...
  win1_data->list_zoom = zoom(ev->mx, ev->my, 3, *win1_data);
  win1_data->zooming = true;
  ez_send_expose(ev->win);
}
...
int main(int argc, char *argv[]) {
  ...
  win1_data.mode = PIXMAP;
  win1_data.save_file = "sauvegarde";
  win1_data.list_zoom = malloc(sizeof(PLISTE));
  win1_data.zooming = false;
